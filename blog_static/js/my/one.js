...
const loginVue = new Vue({
  ...
  methods: {
    async doLogin() {
      if (this.loginData.mail.trim().length == 0 ||
          this.loginData.pwd.trim().length == 0) {
        this.msg = '请输入邮箱或密码'
        return false
      }
      if (this.loginData.mail.includes(' ') || 
...
const registerVue = new Vue({
  ...
  methods: {
    ...
    checkPwd() {
      if (this.registerData.pwd.length == 0 || this.registerData.repeatPwd.length == 0) {
        this.msg = '请填写密码'
        this.isError = true
        return false
      }
      if (this.registerData.pwd.includes(' ') || this.registerData.repeatPwd.includes(' ')) {
        this.msg = '密码中不能包含空格'
        this.isError = true
        return false
      }
      if (this.registerData.pwd.length != this.registerData.repeatPwd.length || this.registerData.pwd != this.registerData.repeatPwd) {
        this.msg = '两次输入的密码不一致'
        this.isError = true
        return false
      }
    ...
    async doRegister() {
      if (this.registerData.writerName == '' ||
...
const blogVue = new Vue({
  ...
  async created() {
    if (location.toString().includes('blog.html')) {
      ...
      if (bid != null && '' != bid) {
        // 查询文章信息
        const {data: res} = await axios.get(baseUrl + '/v2/index/article/view/' + bid)
        if (res.code == 200) {
...
const ppVue = new Vue({
  ...
  async created() {
    if (location.toString().includes('private_personal.html')) {
      ...
      if (res2.code == 200) {
        ...
        this.profile.writer.articleNum = formatData(this.profile.writer.articleNum)
        this.profile.writer.fans = formatData(this.profile.writer.fans)
        this.profile.writer.allLikeNums = formatData(this.profile.writer.allLikeNums)
  ...
  methods: {
    ...
    async doEdit(cmd) {
      // 用户名
      if (cmd == 1) {
        if (this.profile.newWriter.writerName === this.profile.writer.writerName) {
      ...
      // 性别
      if (cmd == 3) {
        if (this.profile.newWriter.writerSex != 'female' && this.profile.newWriter.writerSex != 'male') {
          return
        }
        const {data: res} = await axios({
          url: baseUrl + '/v2/lr/' + this.profile.writer.uid,
      ...
      // 个人介绍
      if (cmd == 5) {
        if (this.profile.newWriter.writerIntroduce == null || this.profile.newWriter.writerIntroduce.trim().length == 0) {
          return
        }
        const {data: res} = await axios({
          url: baseUrl + '/v2/lr/' + this.profile.writer.uid,
...
const indexVue = new Vue({
  ...
  methods: {
    ...
    changehot() {
      ...
      if (this.currentHotBlogPage > this.hotBlogPageInfo.pages) {
        ...
        if (this.hotBlogPageInfo.total > this.currentHotBlogPage * 5) {
          this.hotBlogs = this.hotBlogPageInfo.list.slice((this.currentHotBlogPage-1)*5,this.currentHotBlogPage*5)
        } else {
          this.hotBlogs = this.hotBlogPageInfo.list.slice((this.currentHotBlogPage-1)*5)
        }
      }
    },
    /*
     ...
     */
    async pager(p) {
      const {data: res} = await axios.get(baseUrl + '/v2/index/article/page/' + p)
    ...
    if (location.toString().includes('blog_add.html')) {
      ...
      if (this.token != null && this.token != '') {
        ...
        if (res.code == 522) {
          ...
          const {data: res2} = await axios.get(baseUrl + '/v2/category/listcg', {
            headers: {
              token: this.token
            }
          })
          ...
          const {data: res3} = await axios.get(baseUrl + '/v2/category/listcr', {
            headers: {
              token: this.token
            }
          })
          ...
          this.currentEditArticleId = localStorage.getItem('currentEditArticleId')
          localStorage.removeItem('currentEditArticleId')
          if (this.currentEditArticleId != null) {
            const {data: res3} = await axios({
              url: baseUrl + '/v2/article/' + this.loginUserID + '/' + this.currentEditArticleId,
              method: 'get',
              headers: { token: this.token }
            })
            if (res3.code == 200) {
    ...
    async add(cmd) {
      ...
      if (this.currentEditArticleId != null) {
        ...
        if (res.code == 200) {
          ...
          let t = setInterval(()=>{
            location.href = 'private_personal.html?uid=' + this.loginUserID + '&mfds=' + Math.random()
            clearInterval(t)
          }, 1000)
          return
        }
      ...
      if (res.code == 5052) {
        if (cmd == 1) {
          ...
          location.href = 'private_personal.html?uid=' + this.loginUserID + '&mfsadf=' + Math.random()
        } else {
  ...
  async created() {
    if (location.toString().includes('common_personal.html')) {
      ...
      if (this.currentUserId != this.userInfo.loginUserID) {
        const {data: res} = await axios.get(baseUrl + '/v2/lr/access/' + this.currentUserId)
        if (res.code == 200) {
